...
async def main():
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            if expected_cookie_names.issubset(found_cookie_names):
                print("✓ All test cookies persisted successfully!")
                for cookie_name in expected_cookie_names:
                    print(f"  - {cookie_name}: {cookie_dict2[cookie_name]}")
            else:
                missing_cookies = expected_cookie_names - found_cookie_names
                print(f"✗ Missing cookies: {missing_cookies}")
            
            await browser2.close()
            
        print("Cookie persistence verification completed")
        ...
        # agent_bay.context.delete(context)  # Uncomment if you want to delete the context
