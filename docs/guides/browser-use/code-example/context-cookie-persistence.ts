...
async function main(): Promise<void> {
    ...
    try {
        ...
        if (missingCookies.size === 0) {
            console.log("✓ All test cookies persisted successfully!");
            expectedCookieNames.forEach(cookieName => {
                console.log(`  - ${cookieName}: ${cookieDict2[cookieName]}`);
            });
        } else {
            console.log(`✗ Missing cookies: ${Array.from(missingCookies).join(', ')}`);
        }
        
        await browser2.close();
        console.log("Cookie persistence verification completed");

        // Clean up second session
        console.log("Cleaning up second session...");
        await agentBay.delete(session2, true);
        
        // Clean up context (optional - you might want to keep it for reuse)
        console.log("Cleaning up context...");
        // await agentBay.context.delete(context);  // Uncomment if you want to delete the context
